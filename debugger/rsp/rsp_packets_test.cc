...
namespace {
const rsp::Packet* kNullPacketPtr = NULL;
...
TEST_F(RspPacketTest, UnknownPacket) {
  rsp::Packet* obj = ParseMsg("kaka");
  EXPECT_EQ(kNullPacketPtr, obj);
}
...
TEST_F(RspPacketTest, TypingPacketVisitor) {
  ...
  packs.push_back(new rsp::SetCurrentThreadCommand);
  ...
  for (size_t i = 0; i < packs.size(); i++) {
    ...
    int type0 = vis.type_;
    rsp::Packet* packet2 = packet->Clone();
...
TEST_F(RspPacketTest, SingleWordCommands) {
  ...
  EXPECT_TRUE(TestParseAndPrintout(new rsp::SetCurrentThreadCommand, "Hc-1"));
  EXPECT_TRUE(TestParseAndPrintout(new rsp::SetCurrentThreadCommand, "Hc0"));
  EXPECT_TRUE(TestParseAndPrintout(new rsp::SetCurrentThreadCommand, "Hg-1"));
  EXPECT_TRUE(TestParseAndPrintout(new rsp::SetCurrentThreadCommand, "Hg0"));
  EXPECT_TRUE(TestParseAndPrintout(new rsp::SetCurrentThreadCommand, "Hg138c"));
  EXPECT_TRUE(TestParseAndPrintout(new rsp::SetCurrentThreadCommand, "Hc138c"));
...
TEST_F(RspPacketTest, SetCurrentThreadCommand) {
  rsp::SetCurrentThreadCommand* obj =
      rsp::packet_cast<rsp::SetCurrentThreadCommand>(ParseMsg("Hc-1"));
  ASSERT_NE(kNullPacketPtr, obj);
  EXPECT_EQ(rsp::SetCurrentThreadCommand::FOR_CONTINUE, obj->subtype());
  EXPECT_EQ(-1, obj->thread_id());
  delete obj;

  obj = rsp::packet_cast<rsp::SetCurrentThreadCommand>(ParseMsg("Hc0"));
  ASSERT_NE(kNullPacketPtr, obj);
  EXPECT_EQ(rsp::SetCurrentThreadCommand::FOR_CONTINUE, obj->subtype());
  EXPECT_EQ(0, obj->thread_id());
  delete obj;

  obj = rsp::packet_cast<rsp::SetCurrentThreadCommand>(ParseMsg("Hg-1"));
  ASSERT_NE(kNullPacketPtr, obj);
  EXPECT_EQ(rsp::SetCurrentThreadCommand::FOR_READ, obj->subtype());
  EXPECT_EQ(-1, obj->thread_id());
  delete obj;

  obj = rsp::packet_cast<rsp::SetCurrentThreadCommand>(ParseMsg("Hg0"));
  ASSERT_NE(kNullPacketPtr, obj);
  EXPECT_EQ(rsp::SetCurrentThreadCommand::FOR_READ, obj->subtype());
  EXPECT_EQ(0, obj->thread_id());
  delete obj;

  obj = rsp::packet_cast<rsp::SetCurrentThreadCommand>(ParseMsg("Hg138c"));
  ASSERT_NE(kNullPacketPtr, obj);
  EXPECT_EQ(rsp::SetCurrentThreadCommand::FOR_READ, obj->subtype());
  EXPECT_EQ(0x138c, obj->thread_id());
  delete obj;

  obj = rsp::packet_cast<rsp::SetCurrentThreadCommand>(ParseMsg("Hc138c"));
  ASSERT_NE(kNullPacketPtr, obj);
  EXPECT_EQ(rsp::SetCurrentThreadCommand::FOR_CONTINUE, obj->subtype());
  EXPECT_EQ(0x138c, obj->thread_id());
  delete obj;
...
TEST_F(RspPacketTest, GetThreadInfoReply) {
  ...
  rsp::GetThreadInfoReply* obj =
      rsp::packet_cast<rsp::GetThreadInfoReply>(
          ParseMsg(kMsg, "GetThreadInfo$Reply"));
  ASSERT_NE(kNullPacketPtr, obj);
