    ...
    Dwarf_Error * error)
{
    ...
    do {
        ...
        do {
            /* A DWARF4 segment selector would be first.
               It was not described in DWARF2 or DWARF4, but
               leaving it out made no sense. Thus no change
               in the version number in this section. */

            arange_start_ptr = arange_ptr;
            if( segment_size != 0) {
                READ_UNALIGNED(dbg, range_segment, Dwarf_Addr,
                    arange_ptr, address_size);
                arange_ptr += address_size;
                length = length - address_size;
            }
