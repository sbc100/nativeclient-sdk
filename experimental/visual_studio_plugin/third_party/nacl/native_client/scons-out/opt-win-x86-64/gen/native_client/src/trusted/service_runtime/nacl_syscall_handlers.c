...
/* this function was automagically generated */
static int32_t NaClSysDyncode_CopyDecoder(struct NaClAppThread *natp) {
  struct NaClSysDyncode_CopyArgs {
    uint32_t dest;
    uint32_t src;
    uint32_t size;
  } p = *(struct NaClSysDyncode_CopyArgs *) natp->syscall_args;

  return NaClSysDyncode_Copy(natp, (uint32_t)  p.dest, (uint32_t)  p.src, (uint32_t)  p.size);
}
...
void NaClSyscallTableInit() {
  ...
  NaClAddSyscall(NACL_sys_dyncode_copy, &NaClSysDyncode_CopyDecoder);
}
