...
# This script generates an .nmf file, which provides the mapping to indicate
...
def main(argv):
  ...
  if options.nmf_file == None:
    parser.error("nmf file not specified with -f or --file")
  ...
  # look at each arg, search it to x86_64, x86_32, arm, etc...
  if options.x86_64 != None:
    nmf_file.write('    "x86-64": "' + options.x86_64 + '",\n')
  if options.x86_32 != None:
    nmf_file.write('    "x86-32": "' + options.x86_32 + '",\n')
  if options.arm != None:
    nmf_file.write('    "arm": "' + options.arm + '",\n')
