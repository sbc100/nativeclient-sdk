...
"""Create the NSIS installer fo the SDK."""
...
def MakeNsisInstaller(sdk_version=None, cwd=None):
  '''Create the NSIS installer

  Args:
    sdk_version: A string representing the SDK version.  The string is expected
    to be a '.'-separated triple representing <major>.<minor>.<build>.  If this
    argument is None, then the default is the value returned by
    build_utils.RawVersion()

    cwd: The current working directory.  Various artifacts (such as the NSIS
    installer) are expected to be in this directory.  Defaults to the script's
    directory.
  '''
  if not sdk_version:
